...
public class ToDoList {
    ...
    public void onAddItem() {
        ...
        System.out.println("Item " + newItem.getDescription() + " has been added to the list.");
    }
    ...
    public void onMarkAsDone() {
        ...
        // Check if the itemNumber is valid
        if (itemNumber >= 1 && itemNumber <= user.getToDoItems().size()) {
            // Convert itemNumber to zero-based index
            int index = itemNumber - 1;
            // Get the ToDoItem at the specified index
            ToDoItem item = user.getToDoItems().get(index);
            // Mark the item as done
            item.setDone(true);
            System.out.println("Item marked as done: " + item.getDescription());
        } else {
    ...
    public void onMarkAsUndone() {
        ...
        if (itemNumber >= 1 && itemNumber <= user.getToDoItems().size()) {
            ...
            // Mark the item as done
            item.setDone(false);
            System.out.println("Item marked as undone: " + item.getDescription());
        } else {
            System.out.println("Invalid item number. Please try again.");
        }
    }
    ...
    public void onDeleteItem() {
        ...
        if (itemNumber >= 1 && itemNumber <= user.getToDoItems().size()) {
            ...
            // Remove the ToDoItem at the specified index
            ToDoItem removedItem = user.getToDoItems().remove(index);
            System.out.println("Item deleted: " + removedItem.getDescription());
        } else {
            System.out.println("Invalid item number. Please try again.");
        }
    }
